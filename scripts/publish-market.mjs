...
function main() {
  const renewalOnly = process.argv.includes('--renew-only')
  let manifest
  let artifact
  let homepage
  if (!renewalOnly) {
    ...
    artifact = join('dist-plugins', `${manifest.id}-${manifest.version}.sekplugin`)
    check(existsSync(artifact), `missing exact artifact ${artifact}`)
    homepage = httpsURL(process.env.PLUGIN_HOMEPAGE ?? '', 'PLUGIN_HOMEPAGE').toString().replace(/\/$/, '')
  }
  ...
  try {
    for (let attempt = 1; attempt <= 5; attempt++) {
      ...
      const stagedPaths = renewalOnly ? ['index.json'] : [`plugins/${filename}`, 'index.json']
      run('git', ['-c', 'core.hooksPath=/dev/null', '-C', marketDir, 'add', ...stagedPaths])
      const message = renewalOnly ? `market: renew v3 expiry (sequence ${sequence})` : `${manifest.id}: publish v${manifest.version}`
      if (!commitIfChanged(marketDir, message)) {
        console.log(renewalOnly ? 'Market renewal produced no changes.' : 'Market already contains this exact release.')
        return
      }
      try {
        run('git', ['-c', 'core.hooksPath=/dev/null', '-C', marketDir, 'push', 'origin', 'HEAD:main'])
        console.log(renewalOnly ? `Renewed market expiry with sequence ${sequence}.` : `Published ${filename} with market sequence ${sequence}.`)
        return
      } catch (error) {
