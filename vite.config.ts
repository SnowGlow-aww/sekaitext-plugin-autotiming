...
// Build the auto-timing plugin as a single self-contained ESM bundle.
//
...
// re-export from window.__SEKAI_HOST__. The result has ZERO bare imports, so the
// host can load it via a blob URL on any WebView (no import-map needed).
...
function hostShimPlugin() {
  ...
  return {
    name: 'host-shim',
    // Run BEFORE vite's built-in resolver; otherwise it resolves 'vue' to
    // node_modules and bundles a second Vue copy, breaking the singleton.
...
// In lib mode, `cssCodeSplit: false` still EMITS a separate style.css. The
// .sekplugin only ships entry.js, so fold the CSS back into entry.js as a
// runtime <style> inject, keeping the plugin a true single file.
function cssInjectPlugin() {
  return {
    ...
    generateBundle(_options: unknown, bundle: Record<string, any>) {
      ...
      if (!css) return
      const inject =
        `(function(){try{var d=document,id="sekai-plugin-autotiming-css";` +
        `var s=d.getElementById(id);if(!s){s=d.createElement("style");s.id=id;d.head.appendChild(s);}` +
        `s.dataset.sekaitextPlugin="auto-timing";s.textContent=${JSON.stringify(css)};}catch(e){}})();\n`
